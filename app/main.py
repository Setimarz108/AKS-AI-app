...
def get_enhanced_fallback_response(message: str) -> str:
    ...
    if "inventory" in message_lower:
        return (
            "For FMCG inventory management: Implement ABC analysis "
            "(A-items need daily monitoring), maintain 15-20% safety stock, "
            "target 8-12x annual turnover. Use demand forecasting with "
            "seasonality adjustments and monitor stock-out rates (keep below 2%)."
        )
    elif "customer" in message_lower:
        return (
            "Customer experience optimization: Track NPS scores (target >50), "
            "implement personalization using purchase history, ensure omnichannel "
            "consistency. Focus on reducing customer effort scores. Omnichannel "
            "customers spend 15-35% more than single-channel customers."
        )
    elif "sales" in message_lower:
        return (
            "Sales optimization: Implement dynamic pricing (can boost revenue 2-5%), "
            "use cross-selling at checkout, optimize product placement. Track "
            "conversion rates, average basket size, and customer lifetime value. "
            "A/B test promotional strategies."
        )
    elif "supply" in message_lower:
        return (
            "Supply chain resilience: Diversify suppliers, maintain supplier "
            "scorecards tracking delivery and quality. Keep strategic safety stock "
            "for high-impact products. Target fill rates >98% and perfect order "
            "rates >95%."
        )
    elif "analytics" in message_lower:
        return (
            "Retail analytics dashboard: Include inventory turnover by category, "
            "gross margin trends, customer acquisition cost, lifetime value ratios, "
            "conversion rates by source, and stock-out frequencies. Use cohort "
            "analysis for retention insights."
        )
    else:
        return (
            "I specialize in retail and FMCG optimization. I can provide insights "
            "on inventory management, customer experience, sales optimization, "
            "supply chain, and analytics. What specific area interests you?"
        )
...
async def get_system_metrics():
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            ai_messages = await conn.fetchval(
                "SELECT COUNT(*) FROM chat_history WHERE ai_powered = true"
            )
            avg_confidence = await conn.fetchval(
                "SELECT AVG(confidence) FROM chat_history WHERE ai_powered = true"
            )
            ...
            return {
                ...
                "ai_percentage": (
                    (ai_messages / total_messages * 100) if total_messages > 0 else 0
                ),
